...
class Tile:
    ...
    def _split(self, image, limit):
        ...
        s = image.height // 2
        self.childs[UL] = Tile(image.crop((0, 0, 0 + s, 0 + s)), UL, self.level + 1, self.bb.x + 0, self.bb.y + 0,
                               limit)
        self.childs[UR] = Tile(image.crop((s, 0, s + s, 0 + s)), UR, self.level + 1, self.bb.x + s, self.bb.y + 0,
                               limit)
        self.childs[LL] = Tile(image.crop((0, s, 0 + s, s + s)), LL, self.level + 1, self.bb.x + 0, self.bb.y + s,
                               limit)
        self.childs[LR] = Tile(image.crop((s, s, s + s, s + s)), LR, self.level + 1, self.bb.x + s, self.bb.y + s,
                               limit)
